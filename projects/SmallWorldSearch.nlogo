...
to make-long-range-ties
  ask turtles
  [
    ask other turtles with [not link-neighbor? self] 
    [
      set dist-from-current (abs distance myself) ^ (0 - r)
    ]
    
    create-link-with lottery-winner other turtles with [not link-neighbor? self]
    ]
  
end

to-report lottery-winner [turtleset]
  let pick random-float sum [dist-from-current] of turtleset
  let winner nobody
  ask turtleset
    [ ;; if there's no winner yet...
      if winner = nobody
        [ ifelse dist-from-current > pick
            [ set winner self ]
            [ set pick pick - dist-from-current ] ] ]
  report winner
end
...
The r slider controls whether long range (low r) or short range (high r) links are more likely.  Choose an r and press SETUP (it can take a very long time to setup, depending on the speed of your machine).
...
For what value of r is the average search time lowest? Does it agree with Kleinberg (2000)? (note, Kleinberg's derivation was for an infinite lattice, yours is a finate one, which can make a difference!)
