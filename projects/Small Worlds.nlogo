...
to make-turtles
  crt num-nodes [ set color gray + 2   set root? false ]
...
to rewire-all
  ...
  while [not success?] [
    ;; kill the old lattice,  neighbors, and create new lattice
    ask links [ die ]
    wire-them
    set number-rewired 0
...
to set-turtle-root
  ask turtles [set root? false set color gray + 2 ]
  ask turtle t [set root? true set color red ]
end

to go-DFS
  if not any? links with[rewired? = false] [
  ...
  ask last LIFO [set color orange wait 0.5]
  dfs
end
