to setup
  ca
  ask patches
    [ if pxcor mod 2 = 0 and pycor mod 2 = 1 [set pcolor white]
      if pxcor mod 2 = 1 and pycor mod 2 = 0 [set pcolor white]]
      
  ask n-of number-of-pieces patches
     [ sprout 1  [ set shape one-of ["chess king" "chess rook" "chess bishop" "chess knight" "chess queen" "chess pawn"]
                   set color one-of [red blue]]]
                         
       ask turtles with [shape = "chess pawn" ]   
...
to find-new-heading 
   if shape = "chess king" or shape = "chess queen"   [set heading (45 * (random 8))]
   if shape = "chess rook" or shape = "chess knight"  [set heading one-of [0 90 180 270]]
end

to move
    if shape = "chess king"     [move-one-patch-ahead]
                                 
    if shape = "chess pawn"     [move-one-patch-ahead]
   
    if shape = "chess bishop"   [move-several-patches-ahead] 
    if shape = "chess rook"     [move-several-patches-ahead] 
    if shape = "chess queen"    [move-several-patches-ahead]

    if shape = "chess knight"   [move-knight] 
end

to move-one-patch-ahead  ; pawns and kings
   if patch-at-heading-and-distance heading 1 != nobody
      [fd 1]
end

to move-several-patches-ahead  ; rooks, bishops, queens 
   let i random 8
   if patch-at-heading-and-distance heading i != nobody
      [fd i]                              
end  
